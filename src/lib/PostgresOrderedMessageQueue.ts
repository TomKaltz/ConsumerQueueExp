...
export const PostgresOrderedMessageQueue = <M extends Messages>(
  ...
  const queue: MessageQueue<M> = {
    ...
    enqueue: async (messages) => {
      const N = 3;
      const sql = `
        INSERT INTO "${table}" (name, correlation_key, data)
        VALUES ${messages
          .map(
            (_, i) =>
    ...
    /**
     * Dequeues and pocesses the oldest message from the queue.  The entire correlation key stream is locked
     ...
     * @param opts.correlationKey optional correlation key, if a correlation key is not specified then the oldest message from the queue is dequeued.
     *   If a correlation key is specified then the oldest message from that correlation key stream is dequeued.
     *   The entire correlation key stream of the dequeued message is locked during processing in either case.
    ...
    dequeue: async (callback, opts) => {
      const { correlationKey, leaseMillis = 30000 } = opts || {
        correlationKey: '',
        leaseMillis: 30000
      };
      let acquiredMessage:
      ...
      try {
        ...
        >(
          `WITH lockable_message AS (
             SELECT t.id, t.name, t.correlation_key, t.data, t.created
             FROM "${table}" t
             WHERE ($1::text IS NULL OR t.correlation_key = $1::text)
             AND NOT EXISTS (
               SELECT 1 FROM "${table}" t2
               WHERE t2.correlation_key = t.correlation_key
           ...
           WHERE t.id = l.id
           RETURNING t.id, t.name, t.correlation_key, t.data, t.created`,
          [correlationKey || null, pid, leaseMillis]
        );
        ...
        // if we got here then we have the correlation key stream lock
