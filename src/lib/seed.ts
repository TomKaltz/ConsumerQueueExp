...
(
  id serial PRIMARY KEY,
  name varchar(100) COLLATE pg_catalog."default" NOT NULL,
  correlation_key varchar(100) COLLATE pg_catalog."default" NOT NULL,
  data jsonb NOT NULL,
  created timestamptz NOT NULL DEFAULT now(),
...
-- Partial unique index that enforces only one locked message per correlation_key
CREATE UNIQUE INDEX IF NOT EXISTS "${table}_correlation_key_lock_ix"
  ON public."${table}" (correlation_key)
  WHERE locked_by IS NOT NULL;

-- Index to support the ORDER BY created ASC, id ASC in dequeue
CREATE INDEX IF NOT EXISTS "${table}_dequeue_ix"
  ON public."${table}" USING btree (correlation_key, created ASC, id ASC)
  TABLESPACE pg_default;
